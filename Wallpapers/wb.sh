...
# [Note: This version works on Mac OS X. The MacEarl version may not, depending
#    on your pathnames and whether there are spaces in file or directory names]
#
# Revision 3.0.0
# 1. Modified for use on Mac OS X and configured for use with my favorites
#
# Revision 2.8.1
...
# Enter your password
PASS=yourpassword
# Location of your Wallbase download directory
PREFIX="/Volumes/My_Book_Studio/Pictures/Work/Wallbase"
...
case $QUERY in
Camel_Toe|camel_toe)
    LOCATION="$PREFIX"/Camel_Toe
    COLLECTION=7606
    ;;
Legs|legs)
    LOCATION="$PREFIX"/Legs
    COLLECTION=7602
    ;;
Nature|nature)
    LOCATION="$PREFIX"/Nature
    COLLECTION=7583
    ;;
Natalia|natalia)
    LOCATION="$PREFIX"/Natalia
    COLLECTION=7603
    ;;
Carisha|carisha)
    LOCATION="$PREFIX"/Carisha
    COLLECTION=7599
    ;;
Dasha|dasha)
    LOCATION="$PREFIX"/Dasha
    COLLECTION=7585
    ;;
Breasts|breasts)
    ...
    ;;
Breasts_2|breasts_2)
    LOCATION="$PREFIX"/Breasts_2
    COLLECTION=7581
    ;;
Nudes_5|nudes_5)
    LOCATION="$PREFIX"/Nudes_5
    COLLECTION=7582
    ;;
Nudes_4|nudes_4)
    ...
    ;;
Redheads|Redheads)
    LOCATION="$PREFIX"/Redheads
    COLLECTION=7591
    ;;
Side_Boob|side_boob)
    LOCATION="$PREFIX"/Side_Boob
    COLLECTION=7584
    ;;
Tanlines|tanlines)
    LOCATION="$PREFIX"/Tanlines
    COLLECTION=7586
    ;;
Final_Fantasy|final_fantasy)
    LOCATION="$PREFIX"/Final_Fantasy
    COLLECTION=7607
    ;;
Bijou|bijou)
    LOCATION="$PREFIX"/Bijou
    COLLECTION=7596
    ;;
Keeley|keeley)
    LOCATION="$PREFIX"/Keeley
    COLLECTION=7597
    ;;
Anime_3|anime_3)
	...
	for imgURL in $URLSFORIMAGES
		...
		if cat downloaded.txt | grep -w "$number" >/dev/null
			then
				...
				filename="$(cat $number | grep 'meta name="keywords"' | sed  's .\{35\}  ' | sed 's/.\{4\}$//' | tr -d ',' | gsed -r 's/ background| desktop| best| widescreen| wallpaper|background |desktop |best |widescreen |wallpaper //g')"
